...
const touchOverriddenFiles = () => {
  ...
  sourceFiles.forEach(chromiumSrcFile => {
    ...
    // If the original file doesn't exist, assume that the original files is in the gen dir.
    ...
    if (fs.existsSync(overriddenFile)) {
      ...
      // If the original file is in gen dir, then also check additional gen dirs.
      if (tryOverrideGen && !!additionalGen) {
        overriddenFile = path.join(config.outputDir, additionalGen, 'gen', relativeChromiumSrcFile)
        // Since gen override exists, expecting additional gen file to exist too.
        if (!fs.existsSync(overriddenFile)) {
          throw 'Expected to find [' + overriddenFile + ']. Check if the name of the gen directory\'s parent has changed.'
        }
        updateFileUTimesIfOverrideIsNewer(overriddenFile, chromiumSrcFile)
      }
    }
